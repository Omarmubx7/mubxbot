...
function extractQueryContext(query) {
  ...
  const wantsEmail = /\b(email|e-mail|mail|contact)\b/.test(q);
  const wantsDepartment = /\b(department|dept|section|program|major)\b/.test(q);
  const mentionsOffice = /\b(office|location|room)\b/.test(q);
  const asksWhere = /\bwhere\b/.test(q);
  const context = {
    specificDay: null,
    wantsEmail,
    wantsOffice: mentionsOffice || (asksWhere && !wantsEmail && !wantsDepartment),
    wantsHours: /\b(when|free|available|availability|office hours|hours|schedule|meet|meeting)\b/.test(q),
    wantsDepartment,
...
function generateSmartResponse(professor, query) {
  ...
  // Rule: "what is x email" or "contact x" => email only
  if (context.wantsEmail && !context.wantsOffice && !context.wantsHours && !context.wantsDepartment) {
    return professor.email || 'No email available.';
  }

  // Rule: office/location questions => office code only
  if (context.wantsOffice && !context.wantsEmail && !context.wantsHours && !context.wantsDepartment) {
    return professor.office || 'No office code available.';
  }

  // Rule: "when is x free" => office hours only
  if (context.wantsHours && !context.wantsEmail && !context.wantsOffice && !context.wantsDepartment) {
    return formatOfficeHours(professor, context.specificDay);
  }

  // Rule: department questions => department only
  if (context.wantsDepartment && !context.wantsEmail && !context.wantsOffice && !context.wantsHours) {
