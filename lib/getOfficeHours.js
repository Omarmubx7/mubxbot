...
function extractQueryContext(query) {
  ...
  const context = {
    specificDay: null,
    wantsEmail: /\b(email|e-mail|mail)\b/.test(q),
    wantsOffice: /\b(where\b.*\boffice\b|office\s*(code|location|room)|office\s*number)\b/.test(q),
    wantsHours: /\b(when|free|available|availability|office hours|hours|schedule)\b/.test(q),
...
function generateSmartResponse(professor, query) {
  ...
  // Rule: "what is x email" => email only
  if (context.wantsEmail && !context.wantsOffice && !context.wantsHours) {
    return professor.email || 'No email available.';
  }

  // Rule: "where is x office" => office code only
  if (context.wantsOffice && !context.wantsEmail && !context.wantsHours) {
    return professor.office || 'No office code available.';
  }

  // Rule: "when is x free" => office hours only
  if (context.wantsHours && !context.wantsEmail && !context.wantsOffice) {
